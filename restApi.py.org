...
def bookInfo():
    ...
    res = requests.get(url='https://www.nl.go.kr/NL/search/openApi/search.do',params={'key':KEY, 'kwd':kwd, 'apiType':'json', 'category':'도서', 'srchTarget':request.args.get('srchTarget'), 'pageNum':request.args.get('pageNum'), 'pageSize':request.args.get('pageSize'), 'sort':request.args.get('sort')})
...
def studyInfoSelect():
    ...
    #스터디명 및 스터디등록번호 값이 없으면 전체 스터디 조회 
    if (studyRegNo is None or studyRegNo == '') and (studyNm is None or studyNm) == '': 
        if adminYn == 'Y' :
            sql = "SELECT STUDY_REG_NO AS studyRegNo, STUDY_NM AS studyNm, JOIN_LINK_URL AS joinLinkUrl, LIKES AS likes, VIEWS AS views, U_YN AS uYn, FST_CRT_DTTI AS fstCrtDtti, LT_CH_DTTI AS ltChDtti FROM PD_TB_STUDY_MST WHERE STUDY_REG_NO = 1"
        else :
            sql = "SELECT STUDY_REG_NO AS studyRegNo, STUDY_NM AS studyNm, JOIN_LINK_URL AS joinLinkUrl, LIKES AS likes, VIEWS AS views, U_YN AS uYn, FST_CRT_DTTI AS fstCrtDtti, LT_CH_DTTI AS ltChDtti FROM PD_TB_STUDY_MST WHERE STUDY_REG_NO > 1"
            sql = sql + orderBy
        cur.execute(sql)
    ...
    #스터디 등록번호가 없으면 스터디명으로 LIKE 검색        
    elif (studyRegNo is None or studyRegNo == '') and studyNm:
        if adminYn == 'Y' :
            sql = "SELECT STUDY_REG_NO AS studyRegNo, STUDY_NM AS studyNm, JOIN_LINK_URL AS joinLinkUrl, LIKES AS likes, VIEWS AS views, U_YN AS uYn, FST_CRT_DTTI AS fstCrtDtti, LT_CH_DTTI AS ltChDtti FROM PD_TB_STUDY_MST WHERE STUDY_REG_NO = 1"
            cur.execute(sql)
        else :
            sql = "SELECT STUDY_REG_NO AS studyRegNo, STUDY_NM AS studyNm, JOIN_LINK_URL AS joinLinkUrl, LIKES AS likes, VIEWS AS views, U_YN AS uYn, FST_CRT_DTTI AS fstCrtDtti, LT_CH_DTTI AS ltChDtti FROM PD_TB_STUDY_MST WHERE STUDY_NM LIKE %s"
            sql = sql + orderBy
            cur.execute(sql, ("%" + studyNm + "%"))
        rows = cur.fetchall()
        res_dict = dict(result=[])
        for row in rows :
            sql = "SELECT A.ISBN AS isbn, B.BOOK_TITLE AS bookTitle, B.BOOK_TP_DC AS kdcName, B.AUTHOR AS author, B.COVER_URL AS coverUrl FROM PD_TB_STUDY_DTL A, PD_TB_BOOK_MST B WHERE A.ISBN = B.ISBN AND STUDY_REG_NO = %s"
            ...
            row = row | isbnList_dict 
            res_dict['result'].append(row)
    #스터디명이 없으면 스터디 등록번호로 검색
    else :
        if adminYn == 'Y' :
            #스터디 등록번호로 조회 시 조회수 +1 증가
            sql = "UPDATE PD_TB_STUDY_MST SET VIEWS = VIEWS + 1 WHERE STUDY_REG_NO = 1 ORDER BY VIEWS DESC"
            cur.execute(sql)
            conn.commit()
            sql = "SELECT STUDY_REG_NO AS studyRegNo, STUDY_NM AS studyNm, JOIN_LINK_URL AS joinLinkUrl, LIKES AS likes, VIEWS AS views, U_YN AS uYn, FST_CRT_DTTI AS fstCrtDtti, LT_CH_DTTI AS ltChDtti FROM PD_TB_STUDY_MST WHERE STUDY_REG_NO = 1"
            cur.execute(sql)    
            ...
            #스터디 등록번호로 조회 시 조회수 +1 증가
            sql = "UPDATE PD_TB_STUDY_MST SET VIEWS = VIEWS + 1 WHERE STUDY_REG_NO = %s ORDER BY VIEWS DESC"
            cur.execute(sql, studyRegNo)
            conn.commit()
            sql = "SELECT STUDY_REG_NO AS studyRegNo, STUDY_NM AS studyNm, JOIN_LINK_URL AS joinLinkUrl, LIKES AS likes, VIEWS AS views, U_YN AS uYn, FST_CRT_DTTI AS fstCrtDtti, LT_CH_DTTI AS ltChDtti FROM PD_TB_STUDY_MST WHERE STUDY_REG_NO = %s AND STUDY_REG_NO <> '1'"
            cur.execute(sql, studyRegNo)
...
def studyReplySelect():
    ...
    sql = "SELECT REPLY_REG_NO replyRegNo, STUDY_REG_NO studyRegNo, ISBN isbn, CRT_USID crtUsid, CONTENT content, FST_CRT_DTTI fstCrtDtti, LT_CH_DTTI ltChDtti FROM PD_TB_STUDY_REPLY_INFO "
    where = ""

    # 1) 조회 조건이 없으면 전체 스터디 댓글 조회
    if replyRegNo == '' and isbn == '' and studyRegNo == '' :
        vals = ""
        cur.execute(sql)
    # 2) 댓글등록번호로 조회
    elif replyRegNo :
        where = "WHERE REPLY_REG_NO = %s"
        vals = (str(replyRegNo))
        cur.execute(sql+where, vals)
    # 3) ISBN으로 조회    
    elif isbn :
        where = "WHERE ISBN = %s"
        vals = (isbn)   
    ...
    sql = "SELECT COUNT(*) as totalCount FROM PD_TB_STUDY_REPLY_INFO"
    cur.execute(sql)
    res = cur.fetchone()
